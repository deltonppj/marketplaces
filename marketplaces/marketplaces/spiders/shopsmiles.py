...
from ..items import ShopSmilesItem
...
class ShopsmilesSpider(scrapy.Spider):
    ...
    def __init__(self, search=None, filter=None, price=None, validate_freight=False, **kwargs):
        ...
        self.product_price_sale = './/span[@class="precos-por-section-box "]/span[3]/text()'
        self.product_gain_miles = './/span[contains(@class, "com-clube-smiles")]/span/span[3]/text()'
        ...
        try:
            ...
            while self.is_btn != 'display: none;':
                ...
                if self.web.element_is_present(xpath=self.btn_load_more):
                    ...
                    sleep(3)
                    self.screen_loding = self.web.driver.find_element(By.XPATH,value=self.hide_loding_modal).get_attribute('aria-live')
                    if self.screen_loding == 'polite':
                        # caso ainda esteja carregando, espera 6 segundos
                        sleep(6)
                    self.is_btn = self.web.driver.find_element(By.XPATH, value=self.control_visible_btn).get_attribute('style')  # noqa
                else:
    ...
    def parse(self, response, **kwargs):
        ...
        for card in cards:
            ...
            item['product_sku'] = card.xpath(self.product_url).get().split('&p=')[1].split('&n=')[0].split('_')[0] # Split para pegar o sku do produto # noqa
            item['product_name'] = card.xpath(self.product_name).get()
            item['product_price_sale'] = card.xpath(self.product_price_sale).get()
            item['product_gain_miles'] = card.xpath(self.product_gain_miles).get()
